...
1. `README.md`: 과제 요구사항과 실제 결과
2. `ARCHITECTURE_AND_DATA_MODEL.md`: 최신 구성도와 이벤트·Parquet 데이터 모델
3. `results/assignment6_pipeline_review.json`: 기준·부하·복구 통합 수치
4. `results/assignment6_alert_and_fallback.json`: Alert와 Fallback 실제 판정
5. `logs/fault_invalid_input.log`: `close` 필드 누락으로 발생한 원본 오류
6. `logs/assignment6_alert.log`: 오류 감지와 복구 상태를 읽기 쉽게 정리한 로그
