...
부하를 주지 않기 위해 저장된 실제 Binance BTCUSDT 1분봉을 로컬 Kafka에 재생했습니다.

## 2. 먼저 확인할 파일

1. `ASSIGNMENT6_FINAL_REPORT.md`: 실제 캡처와 전체 결과를 포함한 최종 제출 보고서
2. `README.md`: 과제 요구사항과 재실행 방법
3. `ARCHITECTURE_AND_DATA_MODEL.md`: 최신 구성도와 이벤트·Parquet 데이터 모델
4. `results/assignment6_pipeline_review.json`: 기준·부하·복구 통합 수치
5. `results/assignment6_alert_and_fallback.json`: Alert와 Fallback 실제 판정
6. `logs/fault_invalid_input.log`: `close` 필드 누락으로 발생한 원본 오류
7. `logs/assignment6_alert.log`: 오류 감지와 복구 상태를 읽기 쉽게 정리한 로그
...
| 실제 실패 | `logs/fault_invalid_input.log` | `PROCESS_RETURN_CODE`, `RuntimeError` |
...
`pipeline_code`는 당시 실제 실행 코드를 확인하기 위한 사본입니다. 프로젝트 루트의
`assignment45_submission`에서 다음 명령으로 원본 실험을 다시 실행할 수 있습니다.
...
샘플, 코드, JSON, 로그와 화면 캡처만 포함했습니다. API key, 개인정보, 계정 정보, 5년 대용량
원천 데이터와 실제 주문 기능은 포함하지 않았습니다.
