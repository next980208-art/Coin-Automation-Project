...
flowchart TB
    ...
    subgraph BATCH["과거 데이터 배치 경로"]
        ...
        REST["Binance USDT-M REST API<br/>Airflow가 수집기 실행"]:::done
        ...
        AIRFLOW --> REST --> RAW --> FLINK_BATCH --> PARQUET --> QUALITY
    end

    subgraph LOAD["6차시에서 점검한 로컬 부하·복구 경로"]
        SAVED["저장된 실제 Binance 1분봉<br/>기준 원본 1,000건"]:::done
        REPLAY["Kafka Producer 로컬 재생<br/>1,000 / 파생 10,000 + 중복 500"]:::done
...
```json
{
  ...
  "event_id": "assignment5-load_10000-20260831T014703Z-00000000",
  "event_sequence": 0,
...
| 필드 | 타입 | 역할 |
| --- | --- | --- |
| `schema_version` | string | 실제 Kafka 이벤트 계약 버전 |
| `event_id` | string | 현재 실행 안에서 동일 메시지의 중복을 판단하는 ID |
| `run_id` | string | 기준·부하 실행을 구분하는 실행 ID |
| `event_sequence` | integer | 실행 안에서 이벤트를 정렬하는 순서 |
| `event_time` | string | UTC ISO 8601 형식의 이벤트 발생 시각 |
| `symbol`, `market` | string | 종목과 시장 종류 |
| `open`, `high`, `low`, `close`, `volume` | number | 원본 1분봉 OHLCV |
| `source` | string | 데이터 출처 |
| `source_event_id` | string | 로컬 재생 전 원본 이벤트 추적 ID |

실제 실행에 사용한 `market_event_v1` 메시지에는 `timeframe`이 들어 있지 않습니다. 이번 입력 변환
단계가 1분봉 전용이어서 `prepare_flink_input.py`에서 `1m`을 명시했습니다. 여러 시간 단위를 한
Topic에서 처리할 때는 다음 이벤트 스키마 버전에 `timeframe`을 필수 필드로 추가해야 합니다.

현재 로컬 재생의 `event_id`는 `run_id + sequence`로 생성됩니다. 따라서 이번처럼 같은 실행 안에서
재전송한 중복 500건은 제거하지만, 서로 다른 실행 사이에서 같은 시장 이벤트가 다시 들어오는
경우까지 제거하지는 못합니다. 운영용 다음 버전에서는 `거래소 + 시장 + 종목 + 시간 단위 + 원본
이벤트 시각`으로 `run_id`와 독립적인 결정적 ID를 만들어야 합니다.
...
| `timeframe` | string | 입력 변환 설정 | 이번 실행은 `1m` |
...
| `event_time_ms` | int64 | 표준화 계층 | `timestamp`와 같은 이벤트 발생 시각 |
...
- 추가 보완 필요: Kafka 이벤트에 `timeframe` 포함, 실행 간에도 유지되는 결정적 `event_id`
