...
종료 코드 0은 정상 종료, 0이 아닌 값은 오류 종료를 나타내는 것이 일반적입니다. 이 실험에서는
종료 코드 1이므로 입력 검사가 잘못된 데이터를 발견하고 실행을 중단한 것입니다.
...
시장 이벤트 자체의 고유성을 판단하는 업무 ID입니다.

```text
binance:usdm:BTCUSDT:candle:1m:1787359320000
```

같은 1분봉을 다시 실행해도 event_id는 같아야 중복으로 판단할 수 있습니다.

### `run_id`

프로그램 실행을 구분하는 ID입니다. 같은 event_id를 기준 실행과 부하 실행에서 재사용할 수 있지만
run_id는 실행마다 다릅니다.
