...
def validate_results(report: dict[str, object]) -> list[str]:
    ...
    recovery = report["fault_and_recovery"].get("recovery")
    if not recovery:
        errors.append("Recovery rerun evidence is missing")
    elif int(recovery["flink"]["flink_output_processed_count"]) != int(
        recovery["expected_rows"]
    ):
        errors.append("Recovery output count mismatch")
    return errors
...
def main() -> None:
    ...
    baseline_jsonl = ASSIGNMENT_ROOT / "data" / "baseline_1000_consumed.jsonl"
    fault = run_invalid_input_fault(baseline_jsonl)
    print("[recovery_1000] Reuse validated JSONL and rerun PyFlink", flush=True)
    recovery_csv, recovery_input, recovery_prepare_elapsed = prepare_flink_input(
        "recovery_1000",
        baseline_jsonl,
    )
    recovery_flink = run_flink_batch("recovery_1000", recovery_csv)
    fault["recovery_action"] = "Validated baseline JSONL was prepared and submitted to PyFlink again."
    fault["recovery"] = {
        "name": "recovery_1000",
        "source_path": str(baseline_jsonl.relative_to(PROJECT_ROOT)),
        "expected_rows": scenarios[0]["expected_unique_count"],
        "prepare_elapsed_seconds": round(recovery_prepare_elapsed, 6),
        "input": recovery_input,
        "flink": recovery_flink,
    }
    fault["recovery_output_rows"] = recovery_flink["flink_output_processed_count"]
    ...
    report["validation"] = {
        "expected_checks": [
            ...
            "validated input is rerun after the fault and restores the expected rows",
        ],
        "errors": errors,
        "healthy": not errors,
    }
