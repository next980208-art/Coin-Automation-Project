"""Validate the historical USDT-M futures context Parquet store."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Validate a USDT-M futures context Parquet store.")
    ...
    parser.add_argument("--expected-schema", default="usdm_context_v2_complete_mark")
    parser.add_argument("--report-dir", default="runtime_reports/futures_context_quality")
...
def context_root(args):
    root = Path(args.context_folder)
...
def read_matching_markers(args):
    ...
    if not marker_dir.exists():
        return ranges, [f"Context marker directory does not exist: {marker_dir}"]

    for path in sorted(marker_dir.glob("_SUCCESS_*_CONTEXT_*.json")):
        try:
            payload = json.loads(path.read_text(encoding="utf-8"))
            if payload.get("context_schema_version") != args.expected_schema:
                continue
            ...
            if start >= end or rows < 0:
                errors.append(f"Invalid context marker: {path.name}")
                continue
            ranges.append({"path": str(path), "start": start, "end": end, "rows": rows})
        except (OSError, ValueError, TypeError, json.JSONDecodeError) as error:
            errors.append(f"Unreadable context marker {path.name}: {error}")
...
def marker_chain(ranges, chain_end_date=None):
    ...
    starts_by_end = {}
    for item in ranges:
        existing = starts_by_end.get(item["end"])
        if existing is None or item["start"] < existing["start"]:
            starts_by_end[item["end"]] = item
    ...
    if chain_end_date:
        ...
        if latest_end not in starts_by_end:
            return None, None, [], [f"No context success marker ends at requested chain end: {latest_end.isoformat()}"]
    else:
        latest_end = max(starts_by_end)
    ...
    while boundary in starts_by_end:
        if boundary in visited:
            errors.append("Context marker chain contains a cycle.")
            break
    ...
    if chain_end_date:
        orphan_paths = [
            item["path"]
            for item in ranges
            if item["path"] not in {chain_item["path"] for chain_item in chain} and item["end"] <= latest_end
        ]
        if orphan_paths:
            errors.append("Context markers outside the selected contiguous chain: " + ", ".join(orphan_paths))
    return boundary, latest_end, chain, errors


def validate_files(args, root, range_start=None, range_end=None):
    parquet_paths = sorted(root.glob("context_*.parquet")) if root.exists() else []
    errors = []
    null_counts = {column: 0 for column in REQUIRED_COLUMNS}
    total_rows = 0
    timestamp_parts = []
    imputed_mark_price_rows = 0

    for path in parquet_paths:
        try:
            frame = pd.read_parquet(path)
        except Exception as error:
            errors.append(f"Unreadable context Parquet {path}: {error}")
            continue
        ...
        if set(frame["context_schema_version"].astype(str)) != {args.expected_schema}:
            errors.append(f"Unexpected context schema version in {path}")
        if frame["mark_price"].isna().any():
            errors.append(f"mark_price contains nulls in {path}")
        if "mark_price_status" in frame.columns:
            imputed_mark_price_rows += int((frame["mark_price_status"] != "observed").sum())
        timestamp_parts.append(pd.to_numeric(frame["timestamp"], errors="coerce"))

    if not parquet_paths:
        errors.append(f"No context Parquet files found under {root}")
    if any(value for value in null_counts.values()):
        errors.append("Required context columns contain null values.")
    if not timestamp_parts:
        if range_start and range_end:
            errors.append("No context Parquet rows found in the selected marker-chain range.")
        return parquet_paths, total_rows, null_counts, imputed_mark_price_rows, None, errors
    ...
    return parquet_paths, total_rows, null_counts, imputed_mark_price_rows, (timestamps, unique_timestamps), errors


def build_report(args):
    root = context_root(args)
    ...
    parquet_paths, total_rows, null_counts, imputed_mark_price_rows, timestamp_data, file_errors = validate_files(
        args,
        root,
        chain_start,
        chain_end,
    ...
    report = {
        ...
        "mark_price_imputed_rows": imputed_mark_price_rows,
        "errors": errors,
    }

    if timestamp_data is not None:
        timestamps, unique_timestamps = timestamp_data
        ...
        if duplicate_count:
            errors.append(f"Duplicate context timestamps: {duplicate_count}")
        if gap_count:
            errors.append(f"Context timestamp gaps larger than {args.timeframe}: {gap_count}")

        if chain_start and chain_end:
            expected_rows = int(
                (datetime.combine(chain_end, datetime.min.time()) - datetime.combine(chain_start, datetime.min.time())).total_seconds()
                * 1000
                / step_ms
            )
            ...
            if expected_rows != len(unique_timestamps):
                errors.append("Context timestamp count does not match the contiguous marker range.")
            if marker_rows != total_rows:
                errors.append("Context marker row total does not match Parquet row total.")
...
def main():
    ...
    report_path = report_dir / f"futures_context_quality_{timestamp}.json"
    report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps({"report_path": str(report_path), **report}, ensure_ascii=False, indent=2))
    if not report["healthy"]:
        raise RuntimeError(f"Futures context quality validation failed. See {report_path}")
